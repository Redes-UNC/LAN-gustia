# LAN-gustia

Repositorio destinado a la entrega de los Trabajos Prácticos de la materia **Redes de Computadoras** de la **Facultad de Ciencias Exactas, Físicas y Naturales — Universidad Nacional de Córdoba (UNC)**.

En este repositorio se encuentran las distintas actividades realizados durante el cursado de la materia **Redes de Computadoras — 2026**.

## 📚 Trabajos Prácticos

Cada rama del repositorio corresponde a un Trabajo Práctico diferente.  
Los trabajos se mantienen separados en ramas independientes para facilitar su entrega, revisión y corrección.

## 👥 Integrantes

* Alvarado, Jazmin
* Aroca Bustos, Nahuel Ezequiel 
* Ibarra, Franco Ismael
* Lucero, Fabricio Agustín
* Soto, Milton Joaquin
* Zulaica, Federico José


